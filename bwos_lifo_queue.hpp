 ...
 * @Last Modified time: 2025-12-04 19:44:03
 */
#pragma once
...
enum class LifoQueueErrorCode : uint8_t {
  kSuccess,
  kDone,
  kEmpty,
  kFull,
...
class LifoQueue {
 public:
  explicit LifoQueue(size_t num_blocks, size_t block_size, Allocator allocator = Allocator());
 ...
 private:
  ...
  struct BlockType {
    ...
    LifoQueueErrorCode Put(Tp value) noexcept;
    ...
    alignas(kHardwareDestructiveInterferenceSize) std::atomic<std::uint64_t> head_;
    alignas(kHardwareDestructiveInterferenceSize) std::atomic<std::uint64_t> tail_;
    alignas(kHardwareDestructiveInterferenceSize) std::atomic<std::uint64_t> steal_head_;
    alignas(kHardwareDestructiveInterferenceSize) std::atomic<std::uint64_t> steal_tail_;
  ...
  alignas(kHardwareDestructiveInterferenceSize) std::atomic<size_t> owner_block_{1};
  alignas(kHardwareDestructiveInterferenceSize) std::atomic<size_t> thief_block_{0};
...
LifoQueue<Tp, Allocator>::LifoQueue(size_t num_blocks, size_t block_size, Allocator allocator)
...
template <class Tp, class Allocator>
Tp LifoQueue<Tp, Allocator>::PopBack() noexcept {
  do {
    ...
    if (ec == LifoQueueErrorCode::kSuccess) {
      return value;
    }
    if (ec == LifoQueueErrorCode::kDone) {
      return Tp{};
    }
...
template <class Tp, class Allocator>
Tp LifoQueue<Tp, Allocator>::StealFront() noexcept {
  size_t thief = 0;
  do {
    ...
    while (result.status != LifoQueueErrorCode::kDone) {
      if (result.status == LifoQueueErrorCode::kSuccess) {
        return result.value;
      }
      if (result.status == LifoQueueErrorCode::kEmpty) {
        return Tp{};
      }
      result = block.steal();
    }
...
template <class Tp, class Allocator>
bool LifoQueue<Tp, Allocator>::PushBack(Tp value) noexcept {
  do {
    size_t owner_index = owner_block_.load(std::memory_order_relaxed) & mask_;
    auto &current_block = blocks_[owner_index];
...
template <class Tp, class Allocator>
template <class Iterator, class Sentinel>
Iterator LifoQueue<Tp, Allocator>::PushBack(Iterator first, Sentinel last) noexcept {
  do {
    size_t owner_index = owner_block_.load(std::memory_order_relaxed) & mask_;
    auto &current_block = blocks_[owner_index];
...
template <class Tp, class Allocator>
size_t LifoQueue<Tp, Allocator>::GetFreeCapacity() const noexcept {
...
template <class Tp, class Allocator>
size_t LifoQueue<Tp, Allocator>::GetAvailableCapacity() const noexcept {
  return GetNumBlocks() * GetBlockSize();
}

template <class Tp, class Allocator>
size_t LifoQueue<Tp, Allocator>::GetBlockSize() const noexcept {
  return blocks_[0].block_size();
}

template <class Tp, class Allocator>
size_t LifoQueue<Tp, Allocator>::GetNumBlocks() const noexcept {
  return blocks_.size();
}

template <class Tp, class Allocator>
bool LifoQueue<Tp, Allocator>::AdvanceGetIndex() noexcept {
  size_t owner_counter = owner_block_.load(std::memory_order_relaxed);
...
template <class Tp, class Allocator>
bool LifoQueue<Tp, Allocator>::AdvancePutIndex() noexcept {
  size_t owner_counter = owner_block_.load(std::memory_order_relaxed);
  ...
  if (!next_block.IsWritable()) {  // TODO(victorika): maybe unlikely?
    return false;
  }
  size_t owner_index = owner_counter & mask_;
  auto &current_block = blocks_[owner_index];
...
template <class Tp, class Allocator>
bool LifoQueue<Tp, Allocator>::AdvanceStealIndex(size_t expected_thief_counter) noexcept {
...
template <class Tp, class Allocator>
LifoQueue<Tp, Allocator>::BlockType::BlockType(size_t block_size, Allocator allocator)
    : head_{0}, tail_{0}, steal_head_{0}, steal_tail_{block_size}, ring_buffer_(block_size, allocator) {}

template <class Tp, class Allocator>
LifoQueue<Tp, Allocator>::BlockType::BlockType(const BlockType &other) : ring_buffer_(other.ring_buffer_) {
...
template <class Tp, class Allocator>
typename LifoQueue<Tp, Allocator>::BlockType &LifoQueue<Tp, Allocator>::BlockType::operator=(const BlockType &other) {
  head_.store(other.head_.load(std::memory_order_relaxed), std::memory_order_relaxed);
  tail_.store(other.tail_.load(std::memory_order_relaxed), std::memory_order_relaxed);
  steal_tail_.store(other.steal_tail_.load(std::memory_order_relaxed), std::memory_order_relaxed);
  steal_head_.store(other.steal_head_.load(std::memory_order_relaxed), std::memory_order_relaxed);
...
template <class Tp, class Allocator>
LifoQueue<Tp, Allocator>::BlockType::BlockType(BlockType &&other) noexcept {
  head_.store(other.head_.load(std::memory_order_relaxed), std::memory_order_relaxed);
  tail_.store(other.tail_.load(std::memory_order_relaxed), std::memory_order_relaxed);
  steal_tail_.store(other.steal_tail_.load(std::memory_order_relaxed), std::memory_order_relaxed);
  steal_head_.store(other.steal_head_.load(std::memory_order_relaxed), std::memory_order_relaxed);
...
template <class Tp, class Allocator>
typename LifoQueue<Tp, Allocator>::BlockType &LifoQueue<Tp, Allocator>::BlockType::operator=(
...
template <class Tp, class Allocator>
LifoQueueErrorCode LifoQueue<Tp, Allocator>::BlockType::Put(Tp value) noexcept {
  std::uint64_t back = tail_.load(std::memory_order_relaxed);
  if (back < GetBlockSize()) {  // TODO(victorika): maybe likely
    ring_buffer_[static_cast<size_t>(back)] = static_cast<Tp &&>(value);
    tail_.store(back + 1, std::memory_order_release);
    return LifoQueueErrorCode::kSuccess;
  }
  return LifoQueueErrorCode::kFull;
}

template <class Tp, class Allocator>
template <class Iterator, class Sentinel>
Iterator LifoQueue<Tp, Allocator>::BlockType::BulkPut(Iterator first, Sentinel last) noexcept {
  std::uint64_t back = tail_.load(std::memory_order_relaxed);
...
template <class Tp, class Allocator>
FetchResult<Tp> LifoQueue<Tp, Allocator>::BlockType::Get() noexcept {
  std::uint64_t front = head_.load(std::memory_order_relaxed);
  if (front == GetBlockSize()) {  // TODO(victorika): maybe unlikely?
    return {LifoQueueErrorCode::kDone, nullptr};
  }
  std::uint64_t back = tail_.load(std::memory_order_relaxed);
  if (front == back) {  //  TODO(victorika): maybe unlikely?
    return {LifoQueueErrorCode::kEmpty, nullptr};
  }
  Tp value = static_cast<Tp &&>(ring_buffer_[static_cast<size_t>(back - 1)]);
  tail_.store(back - 1, std::memory_order_release);
  return {LifoQueueErrorCode::kSuccess, value};
}

template <class Tp, class Allocator>
FetchResult<Tp> LifoQueue<Tp, Allocator>::BlockType::Steal() noexcept {
  std::uint64_t spos = steal_tail_.load(std::memory_order_relaxed);
  FetchResult<Tp> result{};
  if (spos == GetBlockSize()) {  //  TODO(victorika): maybe unlikely?
    result.status = LifoQueueErrorCode::kDone;
    return result;
  }
  std::uint64_t back = tail_.load(std::memory_order_acquire);
  if (spos == back) {  //  TODO(victorika): maybe unlikely?
    result.status = LifoQueueErrorCode::kEmpty;
    return result;
  }
  if (!steal_tail_.compare_exchange_strong(spos, spos + 1, std::memory_order_relaxed)) {
    result.status = LifoQueueErrorCode::kConflict;
    return result;
  }
  result.value = static_cast<Tp &&>(ring_buffer_[static_cast<size_t>(spos)]);
  steal_head_.fetch_add(1, std::memory_order_release);
  result.status = LifoQueueErrorCode::kSuccess;
  return result;
}

template <class Tp, class Allocator>
TakeoverResult LifoQueue<Tp, Allocator>::BlockType::Takeover() noexcept {
...
template <class Tp, class Allocator>
bool LifoQueue<Tp, Allocator>::BlockType::IsWritable() const noexcept {
...
template <class Tp, class Allocator>
size_t LifoQueue<Tp, Allocator>::BlockType::FreeCapacity() const noexcept {
  std::uint64_t back = tail_.load(std::memory_order_relaxed);
  return GetBlockSize() - back;
}

template <class Tp, class Allocator>
bool LifoQueue<Tp, Allocator>::BlockType::Reclaim() noexcept {
...
template <class Tp, class Allocator>
size_t LifoQueue<Tp, Allocator>::BlockType::GetBlockSize() const noexcept {
  return ring_buffer_.size();
}

template <class Tp, class Allocator>
void LifoQueue<Tp, Allocator>::BlockType::Grant() noexcept {
...
template <class Tp, class Allocator>
bool LifoQueue<Tp, Allocator>::BlockType::IsStealable() const noexcept {
